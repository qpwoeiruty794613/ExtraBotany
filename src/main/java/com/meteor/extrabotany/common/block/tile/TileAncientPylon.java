...
public class TileAncientPylon extends TileSimpleInventory implements IManaCollector, IWandBindable, IKeyLocked, IThrottledPacket, IManaSpreader, IRedirectable {
	//Copied
	private static final int MAX_MANA = 2000;
	private static final int ULTRA_MAX_MANA = 9800;
	...
	public boolean isRedstone() {
		return worldObj == null ? staticRedstone : getBlockMetadata() == 3;
	}
	...
	public boolean isULTRA_SPREADER() {
		return worldObj == null ? staticUltra : getBlockMetadata() == 1;
	}
	...
	public EntityManaBurst getBurst(boolean fake) {
		...
		int maxMana = ultra ? 760 : dreamwood ? 360 : 240;
		int color = isRedstone() ? 0xFF2020 : dreamwood ? 0xFF45C4 : 0x20FF20;
		int ticksBeforeManaLoss = ultra ? 160 : dreamwood ? 140 : 120;
		float manaLossPerTick = ultra ? 20F : dreamwood ? 8F : 4F;
		float motionModifier = ultra ? 5F : dreamwood ? 3.75F : 1.75F;
		float gravity = 0F;
	...
	public void renderHUD(Minecraft mc, ScaledResolution res) {
		String name = StatCollector.translateToLocal(new ItemStack(com.meteor.extrabotany.common.block.ModBlocks.pylon, 1, getBlockMetadata()).getUnlocalizedName().replaceAll("tile.", "tile." + LibResources.PREFIX_MOD) + ".name");
	...
	public String getInventoryName() {
		return LibBlockName.ANCIENTPYLON;
	}
